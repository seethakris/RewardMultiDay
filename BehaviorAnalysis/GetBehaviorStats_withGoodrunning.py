...
class GetData(object):
    def __init__(self, FolderName, Task_NumFrames, TaskDict, norewardtasks, change_lick_stop=0,
        ...
        # Calculate lick parameters
        self.good_lap_time, self.actual_lap_time = self.find_lap_parameters()
        self.actual_runninglaps, self.lick_perlap_inspace, self.lick_perlap_befclick_inspace, self.lick_perlap_alllicks_inspace, self.licks_bytimefromreward, self.velocity_perlap_inspace, self.velocity_bytime, self.lickperlap, self.lick_bin_edge = self.find_lick_and_attention_parameters()
    ...
    def find_lick_and_attention_parameters(self):
        numBins = np.linspace(0, np.max(self.running_data['Task1']), 40)
        ...
        for i in self.PlaceFieldData:
            ...
            actuallap_run = []
            for this, next in zip(range(1, np.max(lapframes)), range(2, np.max(lapframes) + 1)):
                # print(np.shape(lapframes), np.shape(self.good_running_index[taskname]))
                [thislap, nextlap] = [np.where(lapframes == this)[0], np.where(lapframes == next)[0]]
                ...
                if taskname not in self.NoRewardTasks:
                    try:
                        ...
                        print(np.shape(reward_lap), np.where(np.diff(reward_lap, axis=0) > 4))
                        print(thislap_start, thislap_end)
                    ...
                    subtract_laprun_transition = np.argmax(laprun)
                    laprun_samelap = laprun[:subtract_laprun_transition]
                ...
                velocity = (velocity * 200) / np.max(self.running_data['Task1'])  # convert to cm/s
        ...
        return actuallap_run_dict, numlicks_spacelap_dict, numlicks_befclick_spacelap_dict, numlicks_alllicks_spacelap_dict, numlicks_time_dict, velocity_spacelap_dict, velocity_time_dict, lickperlap_dict, bin_edges
    ...
    def quantify_lickstop_lickperlap(self):
        ...
        highlicks = np.where(np.sum(control_lick, 0) > self.numlaps['Task1'])[
            0]  # If licks exceep numlaps ran by 1.5
        print(np.sum(control_lick, 0))
        try:
            highlicks_space = (self.lick_bin_edge[highlicks[0]] * 200) / np.max(self.running_data['Task1'])
        except:
            highlicks_space = (self.lick_bin_edge[-2] * 200) / np.max(self.running_data['Task1'])
            highlicks = [np.size(self.lick_bin_edge)-2]
        print(f'Space with high licks in 200 cm track : %d cm' % highlicks_space)  # Multiply by track length

        # Find licks in other tasks based on the space of high licks in Task1
        for n, i in enumerate(self.TaskDict.keys()):
            ...
            self.numlicks_withinreward_befclick[i] = np.sum(self.lick_perlap_befclick_inspace[i][:, highlicks[0]:],
                                                            axis=1)
            self.numlicks_withinreward_alllicks[i] = np.sum(self.lick_perlap_alllicks_inspace[i][:, :],
                                                            axis=1)
    ...
    def plot_behavior(self):
        ...
        for n, i in enumerate(self.TaskDict.keys()):
            ...
            axes[n, 1].plot(self.corrected_lick_data[i], linewidth=1, alpha=0.5)
        ...
        fs, axes = plt.subplots(1, figsize=(4, 2), dpi=100)
        axes_count = 0
        for n, i in enumerate(self.TaskDict.keys()):
            if i in self.NoRewardTasks:
                # Divide No reward task based on number of laps before and after lick stops
                axes.bar(axes_count, self.lick_stop[i])
                axes.bar(axes_count + 1, self.numlaps[i] - self.lick_stop[i])
                axes_count += 2

            else:
                axes.bar(axes_count, self.numlaps[i])
                axes_count += 1
        # axes.set_xticks(range(len(self.numlaps) + 1))
        # axes.set_xticklabels(
        #     ('1FamRew', '2FamRewLick', '2FamRewNoLick', '3FamRew', '4NovRew'), rotation=45, ha="right")
    ...
    def plot_velocity_withspaceandtime(self):
        ...
        for n, t in enumerate(self.TaskDict.keys()):
            ...
            axes[n].plot(np.arange(np.size(v1, 1)),
                         np.nanmean(v1, axis=0), 'k', linewidth=2)
            ...
            # axes[1, n].plot(np.linspace(0, 3, 96), self.velocity_byrewardtime[t].T, color='gray')
            # axes[1, n].plot(np.linspace(0, 3, 96), np.mean(self.velocity_byrewardtime[t], 0), color='black', lw=2);
            # axes[1, n].axvline(3, lw=3, color='r')
            # axes[1, n].locator_params(nbins=4)
            # axes[1, n].set_xlabel('Time to reward (s)')
    ...
    def plot_lick_per_lap(self):
        ...
        for n, i in enumerate(self.TaskDict.keys()):
            ...
            axes[0, n].plot(self.numlicks_withinreward_alllicks[i], linewidth=2, marker='.', markersize=10)
            axes[1, n].plot(self.numlicks_withinreward_befclick[i], linewidth=2, marker='.', markersize=10)
            axes[0, n].set_title(self.TaskDict[i])
            if i in self.NoRewardTasks:
                axes[0, n].axvline(self.lick_stop[i], color='k', alpha=0.5, linewidth=2)
                axes[1, n].axvline(self.lick_stop_befclick[i], color='r', linewidth=2)

        axes[1, 0].set_ylabel('Number of pre licks \n in reward zone')
    ...
    def save_beh_parameters(self):
        ...
        np.savez(os.path.join(self.SaveFolder, 'behavior_data.npz'),
                 ...
                 numlicks_perspatialbin=self.lick_perlap_inspace, licks_bytimefromreward=self.licks_bytimefromreward,
                 ...
                 velocity_spatialbins=self.velocity_perlap_inspace, velocity_byrewardtime=self.velocity_bytime,
                 actualrunninglaps=self.actual_runninglaps, lickperlap=self.lickperlap)
