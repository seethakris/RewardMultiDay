...
class LickBehavior(object):
    def __init__(self, ExpFolderName, TaskDict):
        ...
        self.ExpAnimals = [f for f in os.listdir(self.ExpFolderName) if f not in ['.DS_Store']]
    ...
    def plotlickdataaroundreward(self, lick_data, tasks, axis):
        ...
        for t in tasks:
            ...
            axis.plot(x, mean_all, linewidth=0.8, label=self.TaskDict[t])
            axis.fill_between(x, mean_all - std_all, mean_all + std_all, alpha=0.5)
