...
class GetData:
    def __init__(self, animalinfo, FolderName, noreward_task):
        ...
        self.create_populationvector_placecell()
        ...
        for i in self.PlaceFieldData:
            ...
            for n in np.arange(np.size(self.sig_PFs_cellnum_revised[taskname])):
                ...
                for i in np.arange(beg, self.numPFs_incells_revised[taskname][n]):
                    ...
                    pc_temp = np.nanmean(x['sig_PFs'][i][self.sig_PFs_cellnum_revised[taskname][n]], 1)
    ...
    def calculate_remapping_with_task(self, taskA):
        ...
        for i in self.PlaceFieldData:
            ...
            for n, c in enumerate(cells_to_plot):
                if taskname == self.noreward_task:
                    # print(np.nanmean(x['Allbinned_F'][0, c][:, :self.lickstoplap], 1))
        ...
        self.save_pcs(pc_activity_dict, 'pcs_sortedby%s' %taskA)
